        ...
        <div class="policy-section">
            ...
            <div class="policy-controls">
                ...
                <div class="control-buttons-group">
                    ...
                    <div class="run-schedule-group">
                        <UnifiedButton
                            class="run-btn"
                            :variant="runMainButtonVariant"
                            :icon="runMainButtonIcon"
                            :loading="runMainButtonLoading"
                            :disabled="!selectedSchemeId"
                            @click="runMainButtonClick"
                        >
                            {{ runMainButtonText }}
                        </UnifiedButton>
                        <UnifiedButton
                            ...
                            :disabled="!selectedSchemeId || isRunning"
                            :icon="Clock"
                        >
                            定时
                        </UnifiedButton>
...
import { Monitor, VideoPlay, VideoPause, Clock, Setting, CircleCheck, Edit, Close } from '@element-plus/icons-vue'
...
const isRunning = ref(false) // 方案是否正在运行（总策略 需要启动=true）
const isPaused = ref(false) // 方案是否已暂停（总策略 需要启动=false 且 已暂停=true，可继续运行）
const currentPausedGroupName = ref('') // 暂停时正在运行的那一组，便于直观显示「哪组停了」
...
const checkRunningStatus = async () => {
    ...
    try {
        ...
        if (runtimeResponse && runtimeResponse.variables) {
            ...
            // 检查"已暂停"：总策略暂时停下、可继续运行（与“停止”区分，停止后需重新开始）
...
const runSchemeNow = async () => {
    ...
    try {
        ...
        isPaused.value = false
        ElMessage.success(wasPaused ? `方案 ${selectedSchemeId.value} 已继续运行` : `方案 ${selectedSchemeId.value} 已启动`)
        window.dispatchEvent(new CustomEvent('schemeStarted', { detail: { schemeId: selectedSchemeId.value } }))
    } catch (error) {
...
// 暂停方案：关主泵、关当前轮灌组阀门，总策略与灌溉组均显示暂停，已浇量保留；点“继续运行”从断点接着执行
const pauseScheme = async () => {
    ...
    try {
        ...
        isRunning.value = false
        isPaused.value = true
        ElMessage.success(`方案 ${selectedSchemeId.value} 已暂停`)
    } catch (error) {
        ElMessage.error(error.err_msg || '暂停方案失败')
    } finally {
...
// 主按钮（开始/暂停/继续）的文案、样式与点击行为
const runMainButtonText = computed(() => {
    if (isRunning.value) return '暂停'
    if (isPaused.value) return '继续'
    return '开始'
})
const runMainButtonVariant = computed(() => (isRunning.value ? 'danger' : 'primary'))
const runMainButtonIcon = computed(() => (isRunning.value ? VideoPause : VideoPlay))
const runMainButtonLoading = computed(() => {
    if (isRunning.value) return pauseSchemeLoading.value
    return runNowLoading.value
})
const runMainButtonClick = () => {
    if (isRunning.value) pauseScheme()
    else runSchemeNow()
}

...
policy
  ...
  policy '${params.policy_name}'
    ...
    state '浇水'
      ...
      do assignment 'false' '阶段剩余时间' 'prs.variables.get("总灌溉时间") / 1000 / 60 - prs.variables.get("阶段已用时间")/60'
    ...
    state '肥前'
      ...
      do assignment 'false' '阶段已用时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("阶段已用时间") + (Date.now() - prs.variables.get("上次更新时间"))/1000) : prs.variables.get("阶段已用时间")'
      do assignment 'false' '上次更新时间' 'Date.now()'
      do assignment 'false' '阶段剩余时间' 'prs.variables.get("肥前时间") / 1000 / 60 - prs.variables.get("阶段已用时间")/60'
      do assignment 'false' '当前浇水量' 'await prs.getSource("供水流量累计读数") - prs.variables.get("主管道流量累计值")'
      transformer 'timeup'
    ...
    state '施肥'
      ...
      do assignment 'false' '阶段已用时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("阶段已用时间") + (Date.now() - prs.variables.get("上次更新时间"))/1000) : prs.variables.get("阶段已用时间")'
      do assignment 'false' '上次更新时间' 'Date.now()'
      do assignment 'false' '阶段剩余时间' 'prs.variables.get("施肥策略") == "定时" ? (prs.variables.get("施肥时间") / 1000 / 60 - prs.variables.get("阶段已用时间")/60) : ( prs.variables.get("期望施肥总量") > 0 && await prs.getSource("施肥平均流量") > 0 ? ( (prs.variables.get("期望施肥总量") - prs.variables.get("当前施肥量")) / await prs.getSource("施肥平均流量") * 60 : 0)'
      do assignment 'false' '当前浇水量' 'await prs.getSource("供水流量累计读数") - prs.variables.get("主管道流量累计值")'
    ...
    state '肥后'
      ...
      do assignment 'false' '阶段已用时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("阶段已用时间") + (Date.now() - prs.variables.get("上次更新时间"))/1000) : prs.variables.get("阶段已用时间")'
      do assignment 'false' '上次更新时间' 'Date.now()'
      do assignment 'false' '阶段剩余时间' 'prs.variables.get("肥后时间") / 1000 / 60 - prs.variables.get("阶段已用时间")/60'
      do assignment 'false' '当前浇水量' 'await prs.getSource("供水流量累计读数") - prs.variables.get("主管道流量累计值")'
      transformer 'timeup'
        rule 'false' '收尾' 'prs.variables.get("阶段剩余时间") < 0'
        rule 'false' '收尾' 'prs.variables.get("需要跳过") == true'

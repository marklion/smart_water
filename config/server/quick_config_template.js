...
policy
  policy '${params.policy_name}'
    ...
    state '等待阀门'
      ...
      transformer 'timeup'
        rule 'false' '主泵工作' 'prs.variables.get("等待时间") >= prs.variables.get("阀门反应时间")'
      return
      transformer 'next'
...
policy
  policy '${params.policy_name}'
    ...
    state '施肥工作'
      ...
      exit action '${params.farm_name}-施肥泵' 'close'
      enter assignment 'false' '进入时间' 'Date.now()'
...
policy
  ...
  policy '${params.policy_name}'
    ...
    init assignment 'false' '主泵运行中' 'true'
    init assignment 'false' '阶段已用时间' '0'
    init assignment 'false' '上次更新时间' '0'
    init assignment 'false' '当前施肥量' '0'
    init assignment 'false' '当前浇水量' '0'
    init assignment 'false' '阶段剩余时间' '0'
    init assignment 'false' '施肥平均流量缓存' '0'
    ...
    init assignment 'false' '阀门反应时间' '${(params.valve_wait_time_ms != null ? params.valve_wait_time_ms : (params.valve_response_time_ms != null ? params.valve_response_time_ms : 5000))}'
    ...
    watering group matrix 'valve_wait_time' '阀门反应时间'
    ...
    state '空闲'
      ...
      transformer 'next'
        rule 'false' '阀门响应' 'prs.variables.get("需要启动") == true'
      return
    return
    state '阀门响应'
      ${valve_open_config}
      ...
      do assignment 'false' '等待时间' '(Date.now() - prs.variables.get("进入时间"))'
      transformer 'timeup'
        rule 'false' '浇水' 'prs.variables.get("等待时间") >= prs.variables.get("阀门反应时间") && prs.variables.get("是否只浇水") == true'
        rule 'false' '肥前' 'prs.variables.get("等待时间") >= prs.variables.get("阀门反应时间") && prs.variables.get("是否只浇水") != true'
      return
      transformer 'next'
        rule 'false' '空闲' 'prs.variables.get("需要启动") == false'
      return
    ...
    state '浇水'
      enter assignment 'false' '进入时间' 'Date.now()'
      enter assignment 'false' '阶段已用时间' '0'
      enter assignment 'false' '上次更新时间' 'Date.now()'
      enter assignment 'false' '阶段剩余时间' '0'
      do assignment 'false' '阶段已用时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("阶段已用时间") + (Date.now() - prs.variables.get("上次更新时间"))/1000) : prs.variables.get("阶段已用时间")'
      do assignment 'false' '上次更新时间' 'Date.now()'
      do assignment 'false' '阶段剩余时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("总灌溉时间") / 1000 / 60 - prs.variables.get("阶段已用时间")/60) : prs.variables.get("阶段剩余时间")'
    ...
    state '肥前'
      ...
      do assignment 'false' '阶段已用时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("阶段已用时间") + (Date.now() - prs.variables.get("上次更新时间"))/1000) : prs.variables.get("阶段已用时间")'
      do assignment 'false' '上次更新时间' 'Date.now()'
      do assignment 'false' '阶段剩余时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("肥前时间") / 1000 / 60 - prs.variables.get("阶段已用时间")/60) : prs.variables.get("阶段剩余时间")'
      do assignment 'false' '当前浇水量' 'await prs.getSource("供水流量累计读数") - prs.variables.get("主管道流量累计值")'
      transformer 'timeup'
    ...
    state '施肥'
      ...
      enter crossAssignment 'false' '"${params.farm_name}-施肥"' '本次施肥量' '0'
      enter assignment 'false' '进入时间' 'Date.now()'
      enter assignment 'false' '阶段已用时间' '0'
      enter assignment 'false' '上次更新时间' 'Date.now()'
      enter assignment 'false' '阶段剩余时间' '0'
      do crossAssignment 'false' '"${params.farm_name}-施肥"' '需要启动' 'prs.variables.get("主泵运行中")'
      do crossAssignment 'false' '"${params.farm_name}-施肥"' '本次施肥量' 'prs.variables.get("当前施肥量")'
      do assignment 'false' '施肥平均流量缓存' 'await prs.getSource("施肥平均流量")'
      do assignment 'false' '阶段已用时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("阶段已用时间") + (Date.now() - prs.variables.get("上次更新时间"))/1000) : prs.variables.get("阶段已用时间")'
      do assignment 'false' '上次更新时间' 'Date.now()'
      do assignment 'false' '阶段剩余时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("施肥策略") == "定时" ? (prs.variables.get("施肥时间") / 1000 / 60 - prs.variables.get("阶段已用时间")/60) : (prs.variables.get("期望施肥总量") > 0 && prs.variables.get("施肥平均流量缓存") > 0.01 ? Math.min(((prs.variables.get("期望施肥总量") - prs.variables.get("当前施肥量")) / prs.variables.get("施肥平均流量缓存") * 60), 999) : 0)) : prs.variables.get("阶段剩余时间")'
      do assignment 'false' '当前浇水量' 'await prs.getSource("供水流量累计读数") - prs.variables.get("主管道流量累计值")'
    ...
    state '肥后'
      ...
      do assignment 'false' '阶段已用时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("阶段已用时间") + (Date.now() - prs.variables.get("上次更新时间"))/1000) : prs.variables.get("阶段已用时间")'
      do assignment 'false' '上次更新时间' 'Date.now()'
      do assignment 'false' '阶段剩余时间' 'prs.variables.get("主泵运行中") == true ? (prs.variables.get("肥后时间") / 1000 / 60 - prs.variables.get("阶段已用时间")/60) : prs.variables.get("阶段剩余时间")'
      do assignment 'false' '当前浇水量' 'await prs.getSource("供水流量累计读数") - prs.variables.get("主管道流量累计值")'
      transformer 'timeup'
        rule 'false' '收尾' 'prs.variables.get("阶段剩余时间") < 0'
        rule 'false' '收尾' 'prs.variables.get("需要跳过") == true'
    ...
    state '收尾'
      ...
      ${valve_close_config}
      enter crossAssignment 'false' '"${params.farm_name}-施肥"' '需要启动' 'false'
      enter crossAssignment 'false' '"${params.farm_name}-施肥"' '本次施肥量' '0'
      transformer 'next'
        rule 'false' '空闲' 'prs.variables.get("需要启动") == false'
      return
...
policy
  policy '${params.farm_name}-总策略'
    ...
    init assignment 'false' '已暂停' 'false'
    state '空闲'
    ...
    return
    state '工作'
      enter crossAssignment 'false' '"${params.farm_name}-供水"' '需要启动' 'true'
      enter crossAssignment 'false' 'prs.variables.get("当前轮灌组名称")' '主泵运行中' 'true'
      transformer 'next'
        rule 'false' '准备' 'prs.variables.get("当前轮灌组已启动") == false'
        rule 'false' '暂停' 'prs.variables.get("需要启动") == false && prs.variables.get("已暂停") == true'
        rule 'false' '空闲' 'prs.variables.get("需要启动") == false && prs.variables.get("已暂停") != true'
        rule 'false' '空闲' 'prs.variables.get("需要重置") == true'
      return
      transformer 'exp'
        rule 'false' '异常' 'prs.variables.get("供水策略异常") == true'

...
describe('总策略快速配置和验证', () => {
    ...
    test('手动触发总策略', async () => {
        ...
        let start_point = Date.now();
        // 策略由 scan 每 50ms 执行；总策略先给供水设 需要启动/是否由总策略启动，供水才会 空闲→等待阀门。需等至少 1～2 个扫描周期再查状态。
        await wait_ms(150);
        for (let i = 0; i < 6; i++) {
            await cli.run_cmd('policy');
            let policies_lines = (await cli.run_cmd(`list policy 农场1-供水`)).split('\n');
            let status_line_index = policies_lines.findIndex(line => line.startsWith('当前状态'));
            let status_line = status_line_index >= 0 ? policies_lines[status_line_index].split(':')[1].trim() : '(未找到)';
            await wait_ms(100);
            await cli.run_cmd('return');
        }

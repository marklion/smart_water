...
// 与 quick_config_template 默认一致：供水「等待阀门」、轮灌组「阀门响应」需过此时间(ms)才进主泵工作/浇水
...
describe('供水策略快速配置和验证', () => {
    ...
    test('工作时压力变化很多', async () => {
        ...
        const highPressureStatus = await wait_for_policy_status_any('农场1-供水', ['异常停机', '压力异常'], 20000);
        if (highPressureStatus === '压力异常') {
            await trigger_water_policy(false);
            await wait_for_policy_status('农场1-供水', '空闲', 5000);
        }
    });
...
describe('施肥策略快速配置和验证', () => {
    ...
    test('施肥策略正常转两轮', async () => {
        ...
        expect(statistics[0]).toBeCloseTo(0.7, 0);
        expect(statistics[1]).toBeCloseTo(0.4, 0);
    });
...
/** 总策略在「准备」时需 当前轮灌组名称 才能给该组设 需要启动；若 所有轮灌组 未同步到运行时，可先等 准备 再显式设 当前轮灌组名称 */
async function ensure_global_policy_current_group() {
    await wait_for_policy_status('农场1-总策略', '准备', 8000);
    await cli.run_cmd('policy');
    await cli.run_cmd(`runtime assignment 农场1-总策略 false '当前轮灌组名称' '"轮灌组1"'`);
    await wait_ms(60);
    await cli.run_cmd('return');
}
...
describe('总策略快速配置和验证', () => {
    ...
    test('手动触发总策略', async () => {
        ...
        await mock_readout('轮灌阀门2', 5);
        await mock_readout('轮灌阀门3', 2);
        await ensure_global_policy_current_group();
        await wait_for_policy_status('农场1-总策略', '工作', 15000);
        let start_point = Date.now();
        await confirm_valve_status('轮灌阀门1', true);
        await confirm_valve_status('轮灌阀门2', true);
        await confirm_valve_status('轮灌阀门3', false);
        await wait_for_policy_status_any('轮灌组1', ['收尾', '空闲'], 20000);
        // 模版：组1 收尾→总策略 工作→准备；总策略 准备→工作 时(等组2 阀门响应) exit 准备 设组1 需要启动=false→组1 收尾→空闲。先等总策略 工作 再等组1 空闲。
        await wait_for_policy_status('农场1-总策略', '工作', 15000);
        await wait_for_policy_status('轮灌组1', '空闲', 8000);
    ...
    test('总策略过程中有跳过', async () => {
        ...
        await mock_readout('轮灌阀门3', 5);
        await wait_for_policy_status_any('轮灌组1', ['收尾', '空闲'], 20000);
        // 模版：组1 收尾→总策略 工作→准备；总策略 准备→工作 时 exit 准备 设组1 需要启动=false→组1 收尾→空闲。先等总策略 工作 再等组1 空闲。
        await wait_for_policy_status('农场1-总策略', '工作', 15000);
        await wait_for_policy_status('轮灌组1', '空闲', 8000);
        await confirm_policy_status('农场1-总策略', '工作');
        await confirm_policy_status('轮灌组1', '空闲');
...
describe('轮灌组策略快速配置和验证', () => {
    ...
    test('只浇水模式下施肥泵关闭且供水统计正常增加', async () => {
        ...
        // 先等供水从「等待阀门」进入「主泵工作」（阀门响应时间），否则无供水、统计为 0
        await wait_ms(VALVE_RESPONSE_MS + 500);
        ...
        await wait_spend_ms(start_point, 300);

        await confirm_valve_status('农场1-施肥泵', false);

        await mock_total_readout('农场1-主管道流量计', 130);

        // 轮灌组1 阀门响应(5s) + 总灌溉时间 0.07 分钟(≈4200ms)，略多等
